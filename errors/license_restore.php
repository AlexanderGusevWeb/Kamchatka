	...
	jQuery.get('/errors/save_domain_keycode.php', {'keycode':keycode}, function(response) {
		...
		jQuery.get('/errors/save_domain_keycode.php', {'do':'load'}, function (response) {
			...
			if ( errors.length > 0 ) {
				...
				text += '<br/><br/>Попробуйте повторить попытку или обратиться в <a href=\"https://www.umi-cms.ru/support/\" target=\"_blank\">Службу Заботы</a> UMI.CMS.';
			...
			jQuery.get('/errors/save_domain_keycode.php', {'do':'test'}, function (response) {
				...
				if ( critical.length > 0 ) {
					...
					jQuery.each(critical, function(num, error) {
						...
						text += ' Информация об ошибке <a href="https://errors.umi-cms.ru/' + error.getAttribute('code') + '/" target="_blank">' + error.getAttribute('code') + '</a><br/>';
					});
					document.getElementById('license_msg').innerHTML = text;
					return false;
				}
...
function checkLicenseCode(frm) {
	...
	var ip = "<?= isset($_SERVER['SERVER_ADDR']) ? $_SERVER['SERVER_ADDR'] : str_replace("\\", '', $_SERVER['DOCUMENT_ROOT']);?>";
